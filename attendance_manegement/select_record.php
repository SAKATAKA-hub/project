...
//option要素
$now_m = intval($now_dt->format('m')); 
$now_Y = intval($now_dt->format('Y'));
for ($i=0; $i < 12; $i++) {
    $val_Y = $now_m -$i < 1 ? $now_Y -1 : $now_Y;
    $val_m = $now_m -$i < 1 ? $now_m -$i +12 : $now_m -$i;
    $selected = $val_m == $select_m ? "selected" : "";
...
switch ($mode) {
    case 'day_table':  //日別勤怠一覧
        $SQL = "SELECT * FROM {$from} WHERE `in_day`= ? ORDER BY `in_time` ASC";
...
foreach($input_records as $key => $input_record){
    ...
    $employee_id = $input_record["employee_id"]; //従業員ID
    $employee_name = $input_record["employee_name"]; //従業員名
...
//取得情報がなかった時
$table_elements[1] = empty($table_elements[1]) ? "<tr><td colspan ='10'>出勤記録はありません。</td></tr>" : $table_elements[1] ;
...
//取得情報がなかった時
$table_elements[2] = empty($table_elements[2]) ? "<tr><td colspan ='6'>出勤記録はありません。</td></tr>" : $table_elements[2] ;
...
$weeks = array("日","月","火","水","木","金","土");
foreach ($weeks as $wkey => $week){if($in_week_nom == $wkey){$in_week = $week;}}
$table_headings[2] =  sprintf("%d日(%s)",$in_date,$in_week);
$SQL = "SELECT `name` FROM employee_data WHERE `id` = ?";
$DATA = array( $select_employee );
$datas = select_db($SQL,$DATA);  //DBより従業員情報の取得
$table_headings[3] = $datas[0]["name"];
...
switch ($mode) {
    case 'day_table':  //*日別勤怠一覧
        ...
        $table_heading = $table_heading_array['day'];  //表示中の説明
    ...
    case 'month_table': //*月別勤怠一覧
        ...
        $table_heading = $table_heading_array['month'];  //表示中の説明
    ...
    case 'private_table': //*個人別勤怠一覧
        ...
        $table_heading = $table_heading_array['private'];  //表示中の説明
...
$replaces = array(
    ...
    "!table_heading!" => $table_heading, //表示中の説明
...
_pdf_;
$file = "tmpl/attendance_list_pdf.tmpl";//テンプレートファイル
